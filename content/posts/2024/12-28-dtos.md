...
All roads lead **to** the domain, not from it, which is another way of saying
...
people's opinions and I reserve the right to complete change mine. **Take What you Can Find, Find What You Can Take and Find what Works for YOU and YOUR projects**.
...
[^serialize]: the serializer would be default encode the "distance" as an
    JSON object for example, but Sportster would expect it to be an
