...
is no `implements` keyword). Rather they define what a function _needs_.
...
The alternate way of handling an error is to `panic`. As previously mentioned,
panicking functions similarly to an exception, it bubbles-up through the call
stack and can be handled via. the `recover` mechanism. Panic should generally
be used only when an _unexpected_ error occcurs.
...
In the following code example the value is _copied_:
...
func SetUserCredit(user User, amount Int32) {
    user.Credit = amount
}

SetUserCredit(user, 10)

fmt.Printf("%#v", user.Credit) // 0
```

In the following code we use a pointer. When the `SetUserCredit` function is
called it will modify the memory location where `Credit` is stored:
...
am still a beginner [Gopher](https://go.dev/blog/gopher) but I am capable of
being productive and applying most of my existing progamming knowledge to Go.

Variable naming aside, I haven't encountered anything which contradicts my
general approach to programming in PHP.
