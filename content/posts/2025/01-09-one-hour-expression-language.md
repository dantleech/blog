...
{{%  callout %}}
A [binary](https://en.wiktionary.org/wiki/binary) operation is an operations with **two** operands, so for example: `foo or bar` could be `BinaryOp(Variable('foo'), 'or', Variable('bar'))` or `5 ** 6` would be `BinaryOp(Integer(5), '**', Integer(6))`.
